...
public class Recorder {
	...
	private String bodiesFolderPath;
	private String dataFolderPath;
	private String[] staticPatterns = {".*\\.js",".*\\.cache.js",".*\\.css",".*\\.gif",".*\\.jpeg",".*\\.jpg",".*\\.ico",".*\\.woff",".*\\.ttf",".*\\.otf",".*\\.png",".*\\.css?(.*)",".*\\.js?(.*)"};
	...
	public Recorder(int proxyPort, String proxyHost, String tempFilePath, boolean ignoreStatics) {
		...
		if (tempFilePath.charAt(tempFilePath.length()-1)=='/' || tempFilePath.charAt(tempFilePath.length()-1)=='\\') {
		...
		Path path2 = FileSystems.getDefault().getPath("recorder.conf");        
		Logger.getLogger(Recorder.class.getName()).info(path2.toString());
		Option<Path> path = Option.apply(path2);
		Map<String, Object> map = scala.collection.mutable.Map$.MODULE$.<String,Object>empty();
	...
	public String stopAndSave() {
		removePreviousTests();
		String fileName = new StringBuilder(tempFilePath).append('/').append(className).toString();
