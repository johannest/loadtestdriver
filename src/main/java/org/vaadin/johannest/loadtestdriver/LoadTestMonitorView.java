...
public class LoadTestMonitorView extends LoadTestMonitor {
    ...
    private void initializeListeners() {
        ...
        sessionSizeButton.addClickListener(new ClickListener() {
            @Override
            public void buttonClick(ClickEvent event) {
                final double usedMem = getUsedMemoryMB();
                final int sessionCount = SessionListener
                        .getActiveHttpSessions();

                final double deltaMem = usedMem - previousUsedMemory;
                final double deltaSes = sessionCount - previousSessionCount;

                sessionSizeLabel.setValue("E. session size: "
                        + formatter.format(deltaMem / deltaSes));
    ...
    private void createCharts() {
        final Chart memoryChart = new Chart(ChartType.LINE);
        final Chart sessionsChart = new Chart(ChartType.LINE);
        final Chart classesChart = new Chart(ChartType.LINE);
        final Chart threadsChart = new Chart(ChartType.LINE);
    ...
    private void configureChart(Chart chart, String caption) {
        ...
        final PlotOptionsSeries po = new PlotOptionsSeries();
        po.setMarker(new Marker(false));
        chart.getConfiguration().setPlotOptions(po);
    }

    public void update() {
        final boolean shift = sessions.size() > 75;
        ...
        sessions.add(new DataSeriesItem(now,
                SessionListener.getActiveHttpSessions()), true, shift);
    ...
    private String getMaxHeapSizeMB() {
        return Long
                .toString((Runtime.getRuntime().maxMemory()) / (1024 * 1024));
    }
    ...
    private Number getNumberOfClasses() {
        ...
        try {
            ...
            final Vector<Class> classes = (Vector<Class>) f
